...
function getValueText(item) {
  const lastFragmentInput = collectionInput[collectionInput.length - 1];
  const textValue = lastFragmentInput.value;
  const lastFragmentCheckbox = collectonCheckbox[.length - 1];
  const checkboxValue = lastFragmentCheckbox.checked;
}
 */
